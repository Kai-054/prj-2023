...
def find_customer(customers):
    ...
    customer_names, customer_ids, customer_dobs, customer_phones, customer_dates = [], [], [], [], []
    
    for customer in customers:
        customer_names.append(customer.name)
        customer_ids.append(customer.customer_id)
        customer_dobs.append(customer.dob)
        customer_phones.append(customer.phone)
        customer_dates.append(customer.date)

    if name in customer_names:
        print(f"| {'Customer ID': <10} | {'Customer Name': <20} | {'DOB': <10} | {'Phone:' :<15} | {'Join Date': <10} |")
        print("-" * 77)
        for i in range(len(customer_names)):
            if name == customer_names[i]:
                print(f"| {customer_ids[i]:11} | {customer_names[i]:20} | {customer_dobs[i]:10} | {customer_phones[i]:15} | {customer_dates[i]:10} |")
    else:
        print(f"Error! Customer {name} hasn't been found.")
"""def find_customer():
    new_list1 = []
    new_list2 = []
    new_list3 = []
    ...
    if n in arr:
        print(
            f"| {'Customer ID': <10} | {'Customer Name': <20} | {'DOB': <10} | {'Phone:' :<10} | {'Join Date': <10} |")
        print("-" * 77)
        for i in range(0, len(new_list1)):
            # ['hiep' ,'hieu' ,'hiep']
            if n == new_list1[i]:  # hieu = 2
                print(
                    f"| {new_list2[i]:11} | {new_list1[i]:20} | {new_list3[i]:10} | {new_list4[i]:10} | {new_list5[i]:10} |")
    else:
...
def cal_fee():
    ...
    if customer_id in fee_list1 and customer_name in fee_list2:
        ...
        if x in arr1:
            ...
            print(
                f"| {'Customer ID': <10} | {'Customer name': <20} | {'Course ID': <10} | {'Fee (USD)': <10} | {'Pay (USD)': <10} | {'Amount owed': <10} |")
            print("+" + "-" * 89 + "+")
